...
async function main() {
  ...
  const resAdminProfiles = await prisma.resource.upsert({
    where: { id: 'res-admin-profiles' },
    update: {},
    create: {
      id: 'res-admin-profiles',
      moduleId: adminModule.id,
      name: 'Perfiles',
      type: ResourceType.PAGE,
      url: '/admin/profiles',
  ...
  const resAdminModules = await prisma.resource.upsert({
    where: { id: 'res-admin-modules' },
    update: {},
    create: {
      id: 'res-admin-modules',
      moduleId: adminModule.id,
      name: 'Módulos',
      type: ResourceType.PAGE,
      url: '/admin/modules',
  ...
  const resAdminResources = await prisma.resource.upsert({
    where: { id: 'res-admin-resources' },
    update: {},
    create: {
      id: 'res-admin-resources',
      moduleId: adminModule.id,
      name: 'Recursos',
      type: ResourceType.PAGE,
      url: '/admin/resources',
  ...
  const resAdminPerms = await prisma.resource.upsert({
    where: { id: 'res-admin-permissions' },
    update: {},
    create: {
      id: 'res-admin-permissions',
      moduleId: adminModule.id,
      name: 'Permisos',
      type: ResourceType.PAGE,
